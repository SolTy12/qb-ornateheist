...
-- QBCore callback wrapper
local function TriggerCallback(name, cb, ...)
    if not QBCore then
        print("^1[UTK Ornate Heist]^7 ERROR: QBCore not initialized!")
        return
    end
...
function UTK:GetInfo()
    print("^3[UTK Ornate Heist]^7 GetInfo called, triggering callback...")
    TriggerCallback("utk_oh:GetData", function(output)
        if output then
            print("^2[UTK Ornate Heist]^7 Got data from server. Stage: " .. tostring(output.stage))
            self.info = output
            self:HandleInfo()
        else
            print("^1[UTK Ornate Heist]^7 ERROR: No data received from server!")
        end
    end)
end
function UTK:HandleInfo()
    local attempts = 0
    while (not GetPlayerData() or not GetPlayerData().job) and attempts < 50 do
        Citizen.Wait(500)
        attempts = attempts + 1
    end
    ...
    if not PlayerData or not PlayerData.job then
        print("^1[UTK Ornate Heist]^7 ERROR: Failed to get player data!")
        Citizen.Wait(5000)
        self:GetInfo()
        return
    end
    
    print("^3[UTK Ornate Heist]^7 HandleInfo - Job: " .. (PlayerData.job and PlayerData.job.name or "none") .. " | Stage: " .. tostring(self.info.stage))
    
    if PlayerData.job and PlayerData.job.name ~= Config.PoliceJob then
        if self.info.stage == 0 then
            print("^2[UTK Ornate Heist]^7 Stage 0 started - waiting for player at bank")
            print("^3[UTK Ornate Heist]^7 Loud start location: " .. self.loudstart.x .. ", " .. self.loudstart.y .. ", " .. self.loudstart.z)
            print("^3[UTK Ornate Heist]^7 Silent start location: " .. self.silentstart.x .. ", " .. self.silentstart.y .. ", " .. self.silentstart.z)
            Citizen.CreateThread(function()
                ...
                while true do
                    ...
                    if checkCount % 500 == 0 then
                        local dst1 = GetDistanceBetweenCoords(x, y, z, self.loudstart.x, self.loudstart.y, self.loudstart.z, true)
                        local dst2 = GetDistanceBetweenCoords(x, y, z, self.silentstart.x, self.silentstart.y, self.silentstart.z, true)
                        print("^3[UTK Ornate Heist]^7 Player position: " .. string.format("%.2f", x) .. ", " .. string.format("%.2f", y) .. ", " .. string.format("%.2f", z))
                        print("^3[UTK Ornate Heist]^7 Distance to loud start: " .. string.format("%.2f", dst1))
                        print("^3[UTK Ornate Heist]^7 Distance to silent start: " .. string.format("%.2f", dst2))
                    end

                    local dst = GetDistanceBetweenCoords(x, y, z, self.loudstart.x, self.loudstart.y, self.loudstart.z, true)
                    ...
                    if dst <= 6 then
                        ...
                        if dst <= 1.5 and IsControlJustReleased(0,38) then
                            print("^2[UTK Ornate Heist]^7 Starting loud heist... Distance: " .. string.format("%.2f", dst))
                            TriggerCallback("utk_oh:startevent", function(output)
                                print("^3[UTK Ornate Heist]^7 Start event response: " .. tostring(output))
                                if output == true then
                                    UTK.initiator = true
                                    self.info.stage = 1
                                    self.info.style = 1
                                ...
                                elseif output ~= true then
                                    QBCore.Functions.Notify(output, "error")
                                end
                            end, 1)
                        end
                    end
                    ...
                    if dst2 <= 6 then
                        ...
                        if dst2 <= 1.5 and  IsControlJustReleased(0,38) then
                            print("^2[UTK Ornate Heist]^7 Starting silent heist... Distance: " .. string.format("%.2f", dst2))
                            TriggerCallback("utk_oh:startevent", function(output)
                                print("^3[UTK Ornate Heist]^7 Start event response: " .. tostring(output))
                                if output == true then
                                    UTK.initiator = true
                    ...
                    Citizen.CreateThread(function()
                        while true do
                            ...
                            if not UTK.checks.hack1 then
                                local dst = GetDistanceBetweenCoords(x, y, z, self.hack1.x, self.hack1.y, self.hack1.z, true)

                                if dst <= 4 then
                                    DrawText3D(self.hack1.x, self.hack1.y, self.hack1.z, self.text.usehack, 0.40)
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        TriggerCallback("utk_oh:checkItem", function(output)
                                            if output then
                            ...
                            if UTK.checks.hack1 and not UTK.checks.hack2 then
                                local dst = GetDistanceBetweenCoords(x, y, z, self.hack2.x, self.hack2.y, self.hack2.z, true)
                    ...
                    Citizen.CreateThread(function()
                        while true do
                            ...
                            if not UTK.checks.id1 then
                                local dst = GetDistanceBetweenCoords(x, y, z, self.card1.x, self.card1.y, self.card1.z, true)
                                ...
                                if dst <= 4 then
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ...
                                        if output then
                                            ...
                                            self:IdCard()
                                        elseif not output then
                                            QBCore.Functions.Notify("You don't have the ID Card.", "error")
                                        end
                                        end, "id_card")
                                    end
                                end
                            ...
                            if not UTK.searchinfo.found then
                                for i = 1, #UTK.searchlocations, 1 do
                                    if not UTK.searchlocations[i].status then
                                        if GetDistanceBetweenCoords(x, y, z, UTK.searchlocations[i].coords.x, UTK.searchlocations[i].coords.y, UTK.searchlocations[i].coords.z, true) <= 2.2 then
                            ...
                            if UTK.checks.id1 and not UTK.checks.id2 then
                                local dst = GetDistanceBetweenCoords(x, y, z, self.card2.x, self.card2.y, self.card2.z, true)
                    ...
                    Citizen.CreateThread(function()
                        while true do
                            ...
                            if not UTK.checks.thermal1 then
                                local dst = GetDistanceBetweenCoords(x, y, z, self.thermal1.x, self.thermal1.y, self.thermal1.z, true)
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        TriggerCallback("utk_oh:checkItem", function(output)
                                            if output then
                                                ...
                                                self:Plant()
                                            elseif not output then
                                                QBCore.Functions.Notify("You don't have any thermal charges.", "error")
                                            end
                                        end, "thermal_charge")
                                    end
                                end
                            ...
                            if UTK.checks.thermal1 and not UTK.checks.thermal2 then
                                local dst = GetDistanceBetweenCoords(x, y, z, self.thermal2.x, self.thermal2.y, self.thermal2.z, true)
                        ...
                        Citizen.CreateThread(function()
                            while true do
                                ...
                                if not UTK.checks.lockpick1 then
                                    local dst = GetDistanceBetweenCoords(x, y, z, self.lockpick1.x, self.lockpick1.y, self.lockpick1.z, true)
                                        ...
                                        if dst <= 1 and IsControlJustReleased(0, 38) then
                                            TriggerCallback("utk_oh:checkItem", function(output)
                                                if output then
                                                    ...
                                                    self:Lockpick()
                                                elseif not output then
                                                    QBCore.Functions.Notify("You don't have any lockpicks.", "error")
                                                end
                                            end, "lockpick")
                                        end
                                    end
                                ...
                                if not UTK.checks.lockpick2 then
                                    local dst = GetDistanceBetweenCoords(x, y, z, self.lockpick2.x, self.lockpick2.y, self.lockpick2.z, true)
...
function UTK:Plant()
    ...
    QBCore.Functions.Progressbar("thermal_charge", self.text.thermal, 4500, false, true, {
    ...
    Citizen.Wait(4000)
    QBCore.Functions.Progressbar("thermal_burn", self.text.burning, 12000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    ...
    Citizen.Wait(9000)
    QBCore.Functions.Notify(self.text.melted, "success")
    StopParticleFxLooped(effect, 0)
    if self.currentplant == 0 then
        -- Wait for door system to be ready
        local waitTime = 0
        while not PoliceDoors or not PoliceDoors[1] do
            Citizen.Wait(100)
            waitTime = waitTime + 100
            if waitTime > 5000 then -- 5 second timeout
                print("^1[UTK Ornate Heist]^7 ERROR: Door system not ready!")
                QBCore.Functions.Notify("Door system not ready!", "error")
                return
            end
        end
        
        Citizen.Wait(500) -- Additional delay for stability
        TriggerServerEvent("utk_oh:policeDoor", 1, false)
        print("^2[UTK Ornate Heist]^7 Unlocking door 1 after thermal charge")
        self:HandleInfo()
    elseif self.currentplant == 1 then
...
function UTK:Lockpick()
    ...
    QBCore.Functions.Progressbar("lockpick_stage1", self.text.lockpick..": Stage 1", 6000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    ...
    Citizen.Wait(6500)
    QBCore.Functions.Notify("Stage 1 "..self.text.stage, "success")
    SetEntityCoords(ped, loc.x, loc.y, loc.z, 1, 0, 0, 1)
    SetEntityHeading(ped, loc.h)
    TaskPlayAnim(ped, animDict, "a_uncuff", 8.0, 8.0, 6000, 1, 1, 0, 0, 0)
    QBCore.Functions.Progressbar("lockpick_stage2", self.text.lockpick..": Stage 2", 6000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    ...
    Citizen.Wait(6500)
    QBCore.Functions.Notify("Stage 2 "..self.text.stage, "success")
    QBCore.Functions.Notify(self.text.unlocked, "success")
    UTK.disableinput = false
    if self.currentpick == 0 then
        UTK.stage0break = true
...
function UTK:Hack()
    ...
    Citizen.Wait(2000)
    Brute()
    QBCore.Functions.Notify("Open My Computer and navigate to HackConnect.exe", "success")
...
function UTK:IdCard()
    ...
    QBCore.Functions.Progressbar("id_card", UTK.text.card, 2000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    ...
    if self.currentid == 1 then
        UTK.disableinput = false
        QBCore.Functions.Notify(UTK.text.unlocked, "success")
    ...
    elseif self.currentid == 2 then
        ...
        UTK.disableinput = false
        QBCore.Functions.Progressbar("using_id", UTK.text.using, 4000, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
        ...
        repeat
            ...
            count = count - 1
        until count == 0
        QBCore.Functions.Notify(UTK.text.used, "success")
        SpawnObj()
...
function Search(location)
    ...
    QBCore.Functions.Progressbar("search_location", UTK.text.search, 15000, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    ...
    if UTK.searchinfo.random ~= 1 then
        ...
        QBCore.Functions.Notify(UTK.text.nothing, "error")
        UTK.searchinfo.random = math.random(1, UTK.cur - 1)
        UTK.cur = UTK.cur - 1
    else
        UTK.searchinfo.found = true
        QBCore.Functions.Notify(UTK.text.found, "success")
...
function Process(ms, text) 
    QBCore.Functions.Progressbar("custom_progress", text, ms, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        if HasScaleformMovieLoaded(scaleform) and UsingComputer then
            ...
            if IsScaleformMovieMethodReturnValueReady(ClickReturn) then
                ...
                if program == 82 and not Hacking then
                    ...
                    PopScaleformMovieFunctionVoid()
                    Hacking = true
                    QBCore.Functions.Notify("Find the IP adress...", "success")
                elseif program == 83 and not Hacking and Ipfinished then
                    ...
                    Hacking = true
                    QBCore.Functions.Notify("Find the password...", "success")
                ...
                elseif Hacking and program == 84 then
                    ...
                    Hacking = false
                    Ipfinished = true
                    QBCore.Functions.Notify("Run BruteForce.exe", "success")
                ...
                elseif Hacking and program == 86 then
                    ...
                    if UTK.hackmethod == 1 then
                        --TriggerServerEvent("utk_oh:toggleDoor", UTK.hack1.type, vector3(UTK.hack1.x, UTK.hack1.y, UTK.hack1.z), false, UTK.hack1.h)
                        TriggerServerEvent("utk_oh:policeDoor", 3, false)
                        QBCore.Functions.Notify(UTK.text.hacked, "success")
                        UsingComputer = false
                        UTK.disableinput = false
                        UTK.hackfinish = true
                        Ipfinished = false
...
AddEventHandler("utk_oh:policeDoor_c", function(doornum, status)
    if PoliceDoors and PoliceDoors[doornum] then
        PoliceDoors[doornum].locked = status
        -- Apply lock state to door object if it exists
        if PoliceDoors[doornum].obj and DoesEntityExist(PoliceDoors[doornum].obj) then
            FreezeEntityPosition(PoliceDoors[doornum].obj, status)
        end
    else
        print("^1[UTK Ornate Heist]^7 ERROR: PoliceDoors["..doornum.."] is nil!")
    end
end)
...
AddEventHandler("utk_oh:vaultsound", function()
    ...
    if GetDistanceBetweenCoords(x, y, z, UTK.vault.x, UTK.vault.y, UTK.vault.z, true) <= 10 then
        repeat
...
AddEventHandler("utk_oh:vaulttimer", function(method)
    if method == 1 then
        QBCore.Functions.Notify("You have 90 seconds until toxic gas comes in.", "error")
        UTK.starttimer = true
        UTK.vaulttime = 90
        Citizen.CreateThread(function()
            while true do
        ...
        TriggerCallback("utk_oh:gettotalcash", function(result)
            text = "$"..GroupDigits(result)
            QBCore.Functions.Notify("You stole "..text, "success")
        end)
    else
        QBCore.Functions.Notify("You have 90 seconds until toxic gas comes in.", "error")
        UTK.starttimer = true
...
AddEventHandler("utk_oh:gas_c", function()
    Citizen.CreateThread(function()
        UTK.begingas = true
        QBCore.Functions.Notify("Vault door will close automatically in 60 seconds!", "error")
        while true do
            Citizen.Wait(1)
            if UTK.begingas then
                local px, py, pz = GetEntityCoords(PlayerPedId())
                local dst1 = GetDistanceBetweenCoords(px, py, pz, 252.71, 218.22, 101.68, true)
                local dst2 = GetDistanceBetweenCoords(px, py, pz, 262.78, 213.22, 101.68, true)
...
AddEventHandler("utk_oh:policenotify", function(toggle)
    ...
    if player.job and player.job.name == Config.PoliceJob then
        if toggle == 1 then
            QBCore.Functions.Notify("Pacific Standart Bank alarms are triggered!", "inform", 10000)
        ...
        while true do
            if UTK.startloot then
                ...
                if not UTK.checks.grab1 then
                    local dst1 = GetDistanceBetweenCoords(x, y, z, UTK.cash1.x, UTK.cash1.y, UTK.cash1.z, true)
                ...
                if not UTK.checks.grab4 then
                    local dst4 = GetDistanceBetweenCoords(x, y, z, UTK.gold.x, UTK.gold.y, UTK.gold.z, true)
                ...
                if not UTK.checks.grab5 then
                    local dst5 = GetDistanceBetweenCoords(x, y, z, UTK.dia.x, UTK.dia.y, UTK.dia.z, true)
...
Citizen.CreateThread(function()
    ...
    -- Initialize doors
    TriggerCallback("utk_oh:GetDoors", function(result)
        PoliceDoors = result
        print("^2[UTK Ornate Heist]^7 PoliceDoors loaded with " .. #PoliceDoors .. " doors")
    end)
    
    -- Wait for doors to load
    local waitCount = 0
    while not PoliceDoors or #PoliceDoors == 0 do
        Citizen.Wait(100)
        waitCount = waitCount + 1
        if waitCount > 50 then -- 5 seconds timeout
            print("^1[UTK Ornate Heist]^7 ERROR: Failed to load PoliceDoors!")
            return
        end
    end
    
    Citizen.Wait(1000) -- Additional wait for stability
    ...
    while true do
        for k, v in ipairs(PoliceDoors) do
            if PoliceDoors[k].obj == nil or not DoesEntityExist(PoliceDoors[k].obj) then
                ...
                if DoesEntityExist(PoliceDoors[k].obj) then
                    FreezeEntityPosition(PoliceDoors[k].obj, v.locked)
                    print("^3[UTK Ornate Heist]^7 Door " .. k .. " initialized - Locked: " .. tostring(v.locked))
                end
            end
        end
        Citizen.Wait(1000)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        for k, v in ipairs(PoliceDoors) do
            local doorX, doorY, doorZ = v.loc.x, v.loc.y, v.loc.z
            local dst = GetDistanceBetweenCoords(x, y, z, doorX, doorY, doorZ, true)
...
Citizen.CreateThread(function()
    print("^2[UTK Ornate Heist]^7 Client script started!")
    
    -- Wait for Config to load from shared script
    local configWait = 0
    while not Config or not Config.PoliceJob do
        Citizen.Wait(100)
        configWait = configWait + 1
        if configWait > 50 then
            print("^1[UTK Ornate Heist]^7 ERROR: Config not loaded after 5 seconds!")
            Config = Config or {}
    ...
    print("^2[UTK Ornate Heist]^7 Config loaded. PoliceJob: " .. (Config.PoliceJob or "police"))
    
    -- Initialize QBCore
    print("^3[UTK Ornate Heist]^7 Initializing QBCore...")
    local qbAttempts = 0
    while not InitQBCore() and qbAttempts < 100 do
        Citizen.Wait(100)
        qbAttempts = qbAttempts + 1
    end
    
    if not QBCore then
        print("^1[UTK Ornate Heist]^7 CRITICAL ERROR: QBCore not loaded! Script cannot continue.")
        return
    end
    
    print("^2[UTK Ornate Heist]^7 QBCore initialized successfully!")
    
    -- Wait for player to be ready
    print("^3[UTK Ornate Heist]^7 Waiting for player data...")
    local playerData = GetPlayerData()
    local attempts = 0
    while (not playerData or not playerData.job) and attempts < 100 do
        Citizen.Wait(500)
        playerData = GetPlayerData()
        attempts = attempts + 1
        if attempts % 10 == 0 then
            print("^3[UTK Ornate Heist]^7 Still waiting for player data... (" .. attempts .. "/100)")
        end
    end
    
    if playerData and playerData.job then
        print("^2[UTK Ornate Heist]^7 Player data loaded. Job: " .. playerData.job.name)
        PlayerData = playerData
    else
        print("^1[UTK Ornate Heist]^7 WARNING: Player data not fully loaded!")
        print("^1[UTK Ornate Heist]^7 Attempting to continue anyway...")
    end
    
    Citizen.Wait(2000)
    print("^2[UTK Ornate Heist]^7 Starting GetInfo...")
    UTK:GetInfo()
end)
